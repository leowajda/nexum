...
class SiteKitTemplateReferenceLabelTest < SiteKitTestCase
  def test_formats_pattern_label
    label = SiteKit::Templates::ReferenceLabel.call(pattern: { 'label' => 'Graph' })

    assert_equal 'Graph', label
  end

  def test_formats_pattern_label_parts
    parts = SiteKit::Templates::ReferenceLabel.parts(pattern: { 'label' => 'Graph' })

    assert_equal({ 'child' => 'Graph' }, parts)
  end

  def test_formats_pattern_and_variant_without_visual_separator
    label = SiteKit::Templates::ReferenceLabel.call(
      pattern: { 'label' => 'Graph' },
      variant: { 'label' => 'BFS' }
    )
  ...
  def test_formats_variant_label_parts
    parts = SiteKit::Templates::ReferenceLabel.parts(
      pattern: { 'label' => 'Graph' },
      variant: { 'label' => 'BFS' }
    )
