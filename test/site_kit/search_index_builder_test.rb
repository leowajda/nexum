...
class SiteKitSearchIndexBuilderTest < SiteKitTestCase
  ...
  def test_search_index_size_stays_bounded
    ...
    assert(search_records.all? { |record| record.content.length <= SiteKit::SearchRecordFactory::MAX_CONTENT_LENGTH })
  end
  ...
  def search_records
    @search_records ||= SiteKit::SearchIndexBuilder.new(context: build_context, site: build_site).records
  end
