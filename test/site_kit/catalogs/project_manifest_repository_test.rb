...
class SiteKitProjectManifestRepositoryTest < SiteKitTestCase
  def test_loads_project_manifests_from_data_file
    records = YAML.safe_load_file(File.join(SiteKit::Core::Helpers.site_source, '_data', 'projects.yml'))
    manifests = SiteKit::Catalogs::ProjectManifestRepository.new(records).load
  ...
  def test_rejects_duplicate_project_slugs
    ...
    error = assert_raises(SiteKit::Error) do
      SiteKit::Catalogs::ProjectManifestRepository.new(records).load
    end
  ...
  def test_project_registry_filters_optional_missing_sources
    Dir.mktmpdir do |directory|
      ...
      registry = SiteKit::Catalogs::ProjectRegistry.new(records:, repo_root: directory).record
