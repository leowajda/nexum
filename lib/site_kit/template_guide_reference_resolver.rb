  ...
  class TemplateGuideReferenceResolver
    ...
    def references_for_categories(categories)
      matched_references = reference_rules.filter_map do |rule|
        next unless ProblemRules.match?(rule.fetch('problem_rule'), categories)
    ...
    def rule_labels(reference)
      ...
      ProblemRules::RULE_KEYS.flat_map { |key| rule.fetch(key, []) }.uniq
    end
    ...
    def rule_signature(rule)
      ProblemRules::RULE_KEYS.map do |key|
