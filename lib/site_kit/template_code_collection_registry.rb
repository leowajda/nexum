...
module SiteKit
  class TemplateCodeCollectionRegistry
    ...
    def normalize_language_catalog(value)
      Helpers.ensure_hash(value, 'Template language catalog').transform_values do |entry|
        record = Helpers.ensure_hash(entry, 'Template language catalog entry')
        {
          'label' => Helpers.ensure_string(record.fetch('label'), 'Template language catalog entry.label'),
    ...
    def normalize_entry(entry, context, template_id)
      ...
      language_record = language_catalog.fetch(language) do
        raise "#{context}.language references unknown template language '#{language}'"
      end
...

      {
        'entry_id' => entry_id,
        'language' => language,
        'language_label' => Helpers.ensure_string(record.fetch('language_label', language_record.fetch('label')),
                                                  "#{context}.language_label"),
        'code_language' => Helpers.ensure_string(record.fetch('code_language', language_record.fetch('code_language')),
    ...
    def validate_code!(code, context)
      ...
      raise "#{context}.code must not be empty" if stripped_code.empty?

      line_count = stripped_code.lines.size
      raise "#{context}.code must stay within #{MAX_CODE_LINES} lines, got #{line_count}" if line_count > MAX_CODE_LINES

      disallowed_pattern = DISALLOWED_CODE_PATTERNS.find { |pattern| code.match?(pattern) }
      return unless disallowed_pattern

      raise "#{context}.code includes non-template boilerplate matching #{disallowed_pattern.inspect}"
    end

    def validate_entry_id_prefix!(template_id, entry_id)
      return if entry_id.start_with?("#{template_id}-")

      raise "Template '#{template_id}' code entry id '#{entry_id}' must start with '#{template_id}-'"
    end
    ...
    def validate_unique_language_variants!(template_id, entries)
      duplicate_language_variants = Helpers.duplicates(
