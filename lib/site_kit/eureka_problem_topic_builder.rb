...
module SiteKit
  ...
  class EurekaProblemTopicBuilder
    ...
    def validate_problem_categories!
      ...
      raise "Eureka problem categories are not mapped to local topics: #{unknown_categories.sort.join(', ')}"
    end
    ...
    def topic_matches_problem?(topic, category_labels)
      ProblemRules.match_any?(topic.fetch('problem_rules', []), category_labels)
    end
    ...
    def template_reference_resolver
      @template_reference_resolver ||= TemplateGuideReferenceResolver.new(guide: template_guide)
    end
