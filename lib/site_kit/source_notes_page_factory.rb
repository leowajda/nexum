  ...
  class SourceNotesPageFactory
    ...
    def build_document_page(language, module_record, document)
      ...
      build_source_page(
        ...
        source_schema: schema_payload(
          about: [registry_record.fetch("project_title"), module_record.fetch("title"), language.fetch("language_title")],
          breadcrumbs: [
            breadcrumb("Home", "/"),
        ...
        format:,
        structured_data_partial: format == "code" ? STRUCTURED_DATA_SOURCE_DOCUMENT_CODE_PARTIAL : STRUCTURED_DATA_SOURCE_DOCUMENT_PARTIAL
      )
    end
    ...
    def source_page_data(language_slug:, title:, source_header:, source_schema:, source_module:, module_slug: nil, document_url: nil, source_document: nil, format: nil, structured_data_partial: nil)
      {
        "project_slug" => manifest.slug,
