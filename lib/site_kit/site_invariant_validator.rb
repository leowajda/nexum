...
module SiteKit
  class SiteInvariantValidator
    ...
    def validate_rendered_routes!
      duplicate_urls = duplicates(renderable_pages.map(&:url))
      return if duplicate_urls.empty?

      raise "Rendered page URLs must be unique: #{duplicate_urls.join(', ')}"
    end
    ...
    def generated_pages
      @generated_pages ||= site.pages.select { |page| page.is_a?(GeneratedPage) }
    end
