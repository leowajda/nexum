...
module SiteKit
  class TemplateGuideValidator
    ...
    def validate_visible_labels!
      ...
      raise "Human-facing labels must not use '#{VISIBLE_LABEL_SEPARATOR}' as a separator: " \
    ...
    def flowchart_labels
      Helpers.ensure_array(flowchart_data.fetch('nodes'), 'Flowchart data.nodes').flat_map do |node|
    ...
    def validate_flowchart_entrypoints_are_atomic!
      ...
      raise "Flowchart solution nodes must map to one template guide target: #{overloaded_node_ids.sort.join(', ')}"
    end
      ...
      @guide_nodes_by_template ||= record.fetch('patterns').each_with_object({}) do |pattern, result|
        ...
        if pattern_nodes.any?
          ...
          if default_template_id.empty?
            raise "Template guide pattern '#{pattern.fetch('id')}' maps flowchart nodes but has no default template"
          end
