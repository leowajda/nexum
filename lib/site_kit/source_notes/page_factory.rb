    ...
    class PageFactory
      ...
      def build_module_page(language, module_record)
        ...
        build_source_page(
          ...
          title:,
          source_header: payload_builder.header(eyebrow: language.fetch('language_title'), title:),
          source_schema: payload_builder.schema(
            about: [registry_record.fetch('project_title'), language.fetch('language_title'), title],
            breadcrumbs: [
              payload_builder.breadcrumb('Home', '/'),
              payload_builder.breadcrumb(title, module_record.fetch('url'))
            ]
          ),
          source_module: payload_builder.module_page_context(module_record)
        )
      end
      ...
      def build_document_page(language, module_record, document)
        ...
        build_source_page(
          ...
          module_slug: module_record.fetch('module_slug'),
          title:,
          source_header: payload_builder.header(eyebrow: module_record.fetch('title'), title:),
          source_schema: payload_builder.schema(
            about: [registry_record.fetch('project_title'), module_record.fetch('title'),
                    language.fetch('language_title')],
            breadcrumbs: [
              payload_builder.breadcrumb('Home', '/'),
              payload_builder.breadcrumb(module_record.fetch('title'), module_record.fetch('url')),
              payload_builder.breadcrumb(title, document.fetch('route_url'))
            ],
            code_repository: registry_record.fetch('project_source_url'),
            programming_language: language.fetch('language_title')
          ),
          source_module: payload_builder.module_navigation_context(module_record),
          document_url: document.fetch('route_url'),
          source_document: payload_builder.document_context(document),
          format:,
          structured_data_partial: payload_builder.structured_data_partial(format)
        )
      end

      def build_source_page(dir:, page_type:, **data)
        page(dir: dir, page_type: page_type, data: payload_builder.source_page_data(data))
      end
      ...
      def payload_builder
        @payload_builder ||= SiteKit::SourceNotes::PagePayloadBuilder.new(
          manifest: manifest,
          registry_record: registry_record
        )
      end
