  ...
  class ProjectManifestRepository
    ...
    def parse(value, label)
      ...
      unless valid_kinds.include?(kind)
        raise "Project manifest #{label}.kind must be one of: #{valid_kinds.join(', ')}"
      end
      ...
      ProjectManifest.new(
        ...
        homepage_order: Helpers.ensure_integer_or_nil(record["homepage_order"], "Project manifest #{label}.homepage_order") || 999
      )
    end
