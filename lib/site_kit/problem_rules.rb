...
module SiteKit
  module ProblemRules
    ...
    def normalize_labels(value, context)
      Helpers.ensure_array_of_strings(value, context).uniq
    end
    ...
    def normalize_rule(value, context)
      rule = Helpers.ensure_hash(value, context)
      unknown_keys = rule.keys - RULE_KEYS
      raise "#{context} references unsupported keys: #{unknown_keys.join(', ')}" if unknown_keys.any?
      ...
      raise "#{context} must include all or any" unless normalized.key?('all') || normalized.key?('any')
