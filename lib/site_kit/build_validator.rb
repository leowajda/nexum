...
module SiteKit
  class BuildValidator
    ...
    def validate_template_reference_rules!
      ...
      raise "Template guide reference rules use unknown problem categories: #{unknown_labels.sort.join(', ')}"
    end

    def validate_eureka_browser!(browser)
      raise 'Problem explorer filters must not include template patterns' if browser.fetch('filters').key?('patterns')
    ...
    def validate_problem_template_references!(problem)
      disallowed_problem_keys.each do |key|
        raise "Problem '#{problem.fetch('problem_slug')}' must not include #{key}" if problem.key?(key)
      end
    ...
    def validate_template_reference!(problem_slug, reference)
      ...
      unless guide_targets.include?(target)
        raise "Problem '#{problem_slug}' references unknown template guide target '#{target}'"
      end
      return unless reference.fetch('url', '').empty?

      raise "Problem '#{problem_slug}' template reference '#{target}' is missing a URL"
    end
