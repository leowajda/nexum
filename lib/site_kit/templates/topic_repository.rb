  ...
  module Templates
    ...
    class TopicRepository
      ...
      def build_topic(topic, topic_id)
        ...
        flowchart_nodes = flowchart_nodes_for(topic, topic_id)
        ...
        if template_id.empty? && flowchart_nodes.any?
          raise SiteKit::CatalogError, "Algorithmic topic '#{topic_id}' maps flowchart nodes but has no template"
        end
      ...
      def flowchart_nodes_for(topic, topic_id)
        SiteKit::Templates::ProblemRules.normalize_labels(
          topic['flowchart_nodes'] || [],
      ...
      def validate_flowchart_nodes!(topic_records)
        ...
        unless unknown_node_ids.empty?
          raise SiteKit::CatalogError,
                "Algorithmic topics reference unknown flowchart nodes: #{unknown_node_ids.join(', ')}"
        end
        ...
        raise SiteKit::CatalogError,
              "Algorithmic topics must cover every flowchart solution node: #{uncovered_solution_ids.join(', ')}"
      end

      def flowchart_node_ids
        @flowchart_node_ids ||= flowchart_nodes.map { |node| SiteKit::Core::Helpers.ensure_string(node['id'], 'Flowchart node.id') }
      end

      def solution_node_ids
        @solution_node_ids ||= flowchart_nodes.filter_map do |node|
          node_id = SiteKit::Core::Helpers.ensure_string(node['id'], 'Flowchart node.id')
          node_id if node['kind'] == 'solution'
        end
      end

      def flowchart_nodes
        @flowchart_nodes ||= SiteKit::Core::Helpers.ensure_array(flowchart_data.fetch('nodes'),
                                                                 'Flowchart data.nodes').map do |entry|
          SiteKit::Core::Helpers.ensure_hash(entry, 'Flowchart node')
        end
      end
    end
  end
